...
| ~~U7~~ | ~~**sidebar / nav-item**~~ | **Shipped.** Sidebar gains `variant="rail"` (48 px, `bg.subtle`). NavItem gains `variant="icon"` (32×32, position:relative for overlays). Horizontal NavItem default preserved. | ✅ |
...
| ~~C3~~ | ~~**Chip** (+ removable)~~ | **Done.** Absorbed into the renamed `Chip` component (`variant="filter \| removable \| static"`). Replaces the former `FilterButton`; filter chips, removable tags, and static labels share one primitive. | `<Chip variant="filter\|removable\|static" active? onRemove? leftIcon?>…</Chip>` | ✅ |
...
| ~~C6a~~ | ~~**ListPagination**~~ | **Shipped.** Prev/next chevrons + range label (`1–25 sur 30`), 24 px buttons, top border subtle. Used in list panel footers. | `<ListPagination page total pageSize onChange formatLabel?/>` | ✅ |
| C6b | **TablePagination** | Numbered pages, page-size selector, ellipsis. For table footers. Separate component — APIs diverge too much from ListPagination. | `<TablePagination page total pageSize pageSizeOptions onChange onPageSizeChange/>` | M |
...
| ~~C10~~ | ~~**NotificationBadge**~~ | **Shipped.** Absolutely-positioned overlay pill (14×14 min, `status.error` bg, 2 px `bg.subtle` halo). `count`, `max`, `showZero` props. Hides at count=0 by default. | `<NotificationBadge count={25} max={99}/>` | ✅ |
