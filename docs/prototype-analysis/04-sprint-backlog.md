...
| ~~C6~~ | ~~**Pagination**~~ | **Shipped (`size="sm"` only).** Prev/next chevrons + range label (`1–25 sur 30`), 24 px buttons, top border subtle. `size="md"` with numbered pages will follow when needed for table views. | `<Pagination page total pageSize onChange formatLabel?/>` | ✅ |
